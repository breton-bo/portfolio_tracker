Imports yFinance API for recieving stock market information
```python
import yfinance as yf
```
A loop for recieving valid user input to store as stock vallues for portfolio computation and printing out stock information
```python
...
while True:
    ...
    if break_loop.lower() in ["yes","y"]:
        break
Grabs live stock information from yFinance API and performs calculations for value and profit margins of each individual stock
```python
...
Pulls stock data from the stock list built by the user and combines all the stocks to create an overall portfolio value and profit margin
```python
